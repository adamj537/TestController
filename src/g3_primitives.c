...
/* ── dut_read_product_ver ─────────────────────────────────────────────────── *
 *
 * Read product firmware version from DUT flash via SWD.
 * Checks magic word at 0x08000200 — 0xC0DEBABE means PFW is still loaded.
 * If product FW is present, reads st_GlobHead_t at 0x08007FC0 for version:
 *   word0[31:24] = fw_issue, word1[7:0] = fw_release, word1[15:8] = fw_point.
 * Always records as pass (informational — does not gate subsequent steps).
 *
 * DUT must be powered before calling.
 *
 * No params.
 */
void run_dut_read_product_ver(const cJSON *params)
{
    (void)params;

    uint32_t magic = 0;
    int rc = swd_read_mem32(0x08000200, &magic);
    if (rc != 0) {
        printf("[INFO] dut_read_product_ver: SWD read failed (rc=%d)\n", rc);
        selftest_check_record("dut_read_product_ver", true);  /* informational */
        return;
    }

    if (magic == 0xC0DEBABE) {
        printf("[INFO] dut_read_product_ver: PFW still loaded (magic=0xC0DEBABE)"
               " — product FW not present\n");
        selftest_check_record("dut_read_product_ver", true);  /* informational */
        return;
    }

    uint32_t word0 = 0, word1 = 0;
    bool swd_ok = (swd_read_mem32(0x08007FC0, &word0) == 0 &&
                   swd_read_mem32(0x08007FC4, &word1) == 0);
    if (!swd_ok) {
        printf("[INFO] dut_read_product_ver: could not read GlobHead\n");
        selftest_check_record("dut_read_product_ver", true);  /* informational */
        return;
    }

    uint8_t fw_issue   = (uint8_t)((word0 >> 24) & 0xFF);
    uint8_t fw_release = (uint8_t)(word1 & 0xFF);
    uint8_t fw_point   = (uint8_t)((word1 >> 8) & 0xFF);
    printf("[INFO] dut_read_product_ver: product FW %u.%u.%u\n",
           fw_issue, fw_release, fw_point);

    selftest_check_record("dut_read_product_ver", true);  /* informational */
}

/* ── vdut_off ──────────────────────────────────────────────────────────────── *
 * Cut TC-side VDUT1 power.  Use as the final recipe step to leave the DUT
 * unpowered at end of test.
 *
 * No params.
 */
void run_vdut_off(const cJSON *params)
{
    (void)params;
    vdac_disable(0);
    printf("[INFO] vdut_off: VDUT1 disabled\n");
    selftest_check_record("vdut_off", true);
}
