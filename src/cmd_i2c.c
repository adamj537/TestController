...
bool i2c_probe(uint8_t addr)
{
    ...
    return i2c_master_probe(s_bus, (uint16_t)addr, 20) == ESP_OK;
}

bool i2c_reinit(int sda, int scl)
{
    if (s_bus != NULL) {
        i2c_del_master_bus(s_bus);
        s_bus = NULL;
...
bool i2c_write_reg(uint8_t addr, uint8_t reg, uint8_t val)
{
    ...
    esp_err_t err = i2c_master_transmit(dev, buf, 2, 200);
    i2c_master_bus_rm_device(dev);
    return err == ESP_OK;
}

bool i2c_write_reg16(uint8_t addr, uint8_t reg, uint16_t val)
{
    if (s_bus == NULL) return false;
    i2c_device_config_t dev_cfg = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
    ...
    esp_err_t err = i2c_master_transmit(dev, buf, 3, 200);
    i2c_master_bus_rm_device(dev);
    return err == ESP_OK;
}
...
bool i2c_read_reg(uint8_t addr, uint8_t reg, uint8_t *buf, int len)
{
    ...
    i2c_master_dev_handle_t dev;
    if (i2c_master_bus_add_device(s_bus, &dev_cfg, &dev) != ESP_OK) return false;
    esp_err_t err = i2c_master_transmit_receive(dev, &reg, 1, buf, (size_t)len, 200);
    i2c_master_bus_rm_device(dev);
    return err == ESP_OK;
}
